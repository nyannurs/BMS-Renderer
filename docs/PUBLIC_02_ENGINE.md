...
`render_bga_video_job(job)` — `job = (in_path, out_path, ffmpeg, library_root, fps, size [, encode_opts])`. It renders the audio, builds the base and overlay BGA timelines, and for each output frame composites the overlay (channel 07) over the base (channel 04) treating black as transparent, then pipes raw frames to ffmpeg with the audio muxed in. The returned output path reflects the actual file written (which may be `.mkv`).
